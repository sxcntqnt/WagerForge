...
async function launchBrowser() {
  if (!browser) {
    // Read browser path from .env or fallback to system default
    ...
    try {
      ...
      logger.info(`✅ Puppeteer browser launched successfully using path: ${browserPath}`);
    } catch (error) {
      logger.error(`❌ Failed to launch Puppeteer browser with path "${browserPath}": ${error.message}`);
      throw error;
    }
  }
  return browser;
