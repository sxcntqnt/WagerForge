...
export class NATSListener {
  constructor(config) {
    ...
    this.nc = null;
    this.currentConfig = config; // Pass config during initialization
  }
  ...
  periodicCleanup() {
    setInterval(() => {
      ...
      for (const [id, store] of [...this.arbCycleStore, ...this.ackStore, ...this.returnsStore]) {
        if (now - store.receivedAt > timeoutMs) {
          logger.warn(`Timeout for ${id} in store. Received ${store.bets?.length || store.acks?.length || store.returns?.length} items.`);
          this.arbCycleStore.delete(id);
          this.ackStore.delete(id);
          this.returnsStore.delete(id);
        }
      }
    }, 10 * 1000);
  }
