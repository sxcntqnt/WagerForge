...
export class SportsbookApp {
  constructor({ page, config, nc, publisher, tracker }) {
  ...
  async placeBet(betRequest) {
    ...
    if (this.balanceTracker) {
      console.log("Refreshing balance after bet confirmation...");
      const newBalance = await this.balanceTracker.refreshBalances();
