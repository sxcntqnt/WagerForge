...
import logger from './logger.js'; // Replace with console if no custom logger
...
export class GitHubRepoMonitor {
  ...
  async downloadAndExtract(ref = 'main') {
    ...
    try {
      ...
      const localFolderRoot = path.join(this.localBasePath, ref); // Store under commit SHA or ref
      ...
      for (const entry of zipEntries) {
        ...
        const relPath = name.substring(basePrefix.length + 1); // Relative to repo root
        if (!relPath) continue;
  ...
  async getLatestConfigDetails() {
    ...
    for (const subdir of subdirs) {
      ...
      if (!config.sportsbooks || !Array.isArray(config.sportsbooks)) {
        this.logger.warn(`Invalid config in ${configPath}: 'sportsbooks' must be an array`);
        continue;
      }
      configs.push({
        path: configPath,
